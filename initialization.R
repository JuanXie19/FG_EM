...
### initiallization opt1: c_hat, r_hat, tau_hat, sigma_Sq the same as above, just use different initialization for phi
## this is the version I used at the beginning
initial_kmeans_opt1 <- function(x, M){
  n <- nrow(x)
  pi_hat <- rep(1/M, M)
  set.seed(12345)
  init_kmeans <- kmeans(x, centers = M, nstart = 1000, algorithm = 'Lloyd')
  ...
  # Normalize the cluster centers c_hat to initialize phi_hat
  phi_hat <- t(apply(c_hat, 1, function(x) x / norm_vec(x)))  #
